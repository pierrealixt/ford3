...
class ProviderFormTest(TestCase):
    ...
    def test_form_validation_for_blank_items(self):
        ...
        self.assertEqual(form.errors['physical_address_city'],
                         [EMPTY_MESSAGE_ERROR])
        self.assertEqual(form.errors['physical_address_postal_code'],
                         [EMPTY_MESSAGE_ERROR])
        self.assertEqual(form.errors['postal_address_line_1'],
                         [EMPTY_MESSAGE_ERROR])
        self.assertEqual(form.errors['postal_address_city'],
                         [EMPTY_MESSAGE_ERROR])
        self.assertEqual(form.errors['postal_address_postal_code'],
                         [EMPTY_MESSAGE_ERROR])
    ...
    def test_form_validation_for_max_length(self):
        form = ProviderForm(
            data={'telephone': '0821234123412341234',
                  ...
                  'physical_address_postal_code': '12345'})
        self.assertFalse(form.is_valid())
        try:
        ...
        try:
            self.assertIn('Ensure this value has at most 4 characters',
                          str(form.errors['physical_address_postal_code'][0]))
        except KeyError:
